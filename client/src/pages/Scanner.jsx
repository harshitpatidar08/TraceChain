...
const Scanner = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 pt-12 font-sans font-medium">
      <div className="max-w-xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-400">Scan QR codes or labels to trace origins securely.</p>
        </div>
        ...
        <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-lg">
          <form onSubmit={handleManualSubmit} className="flex gap-3 mb-6">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full bg-slate-900 border border-slate-700 focus:border-orange-500 rounded-lg py-3 pl-12 pr-4 transition-colors font-mono uppercase text-sm"
              />
            </div>
          ...
          <div className="pt-6 border-t border-slate-700 flex flex-col items-center">
            ...
            <button 
              ...
              disabled={ocrLoading}
              className="w-full bg-slate-700 hover:bg-slate-600 text-white rounded-lg py-3 border border-slate-600 font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
            >
              {ocrLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <ImageIcon className="w-5 h-5" />}
              Scan Product Label (OCR)
            </button>
            <p className="text-xs text-slate-400 mt-3 text-center">Take a photo of product packaging to automatically extract Trace IDs using AI vision.</p>
          </div>
        </div>
            ...
            <div className="space-y-3">
              {recentScans.map((scan, idx) => (
                <div 
                  ...
                  className="bg-slate-800 p-4 rounded-xl border border-slate-700 hover:border-orange-500/50 cursor-pointer transition-all flex justify-between items-center group"
                >
                  <div>
                    <h4 className="font-semibold text-white group-hover:text-orange-400 transition-colors">{scan.traceId}</h4>
                    <p className="text-xs text-slate-400 font-mono mt-1">Source: {scan.name}</p>
                  </div>
